...
const id=params.get('id'),revision=Number(params.get('revision')),token=params.get('choice')==='details'?null:params.get('token');
let choice=params.get('choice');
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const api=token?'/api/landlord-decision':`/api/admin/cases/${id}`;
const options=()=>({credentials:'same-origin',headers:token?{Authorization:`Bearer ${token}`}:{}});
...
function signIn(wrong=false){
 host.innerHTML=`<h1>${wrong?'Landlord Account Required':'Confirm Your Rental Decision'}</h1><p>${wrong?'This link requires the assigned landlord account. Sign in with the email this invitation was sent to, or contact the leasing team.':'Sign in with the landlord email this summary was sent to. '}</p><a class="button" href="/login/?return=${encodeURIComponent(location.pathname+location.hash)}">Sign In to Continue</a>`;
}
function render(row){
 ...
 <p>Review the application and confirm your decision below.</p>
 ...
 ${token?`<p><a class="button secondary" href="#id=${encodeURIComponent(id)}&revision=${revision}&choice=details" data-view-details>View Details</a></p>`:`<details class="applicant-details" ${choice==='details'?'open':''}><summary>Applicant Summary</summary><div class="people">${(r.members || []).map(m=>`<section><h2>${esc(m.name)}</h2><small>Credit score ${m.mock?'(mock)':''}</small><b class="score">${esc(m.credit_score ?? 'No score returned')}</b><small>${esc(m.score_model)} · ${esc(m.report_date?.slice(0,10))}</small><p>Annual income: <b>${esc(m.annual_income || 'Not stated')}</b><br><small>${esc(m.income_source)}</small></p><p>${esc(m.employment)}</p><small>Report: ${esc(m.report_status)}</small></section>`).join('')}</div></details>`}
 ...
 host.querySelector('[data-view-details]')?.addEventListener('click',()=>{setTimeout(()=>location.reload(),0);});
 host.querySelector('[data-change]')?.addEventListener('click',()=>{choice='';render(row);});
...
async function start(){
 ...
 if([401,403].includes(response.status)){signIn(response.status===403);return;}
