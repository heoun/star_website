...
export function groupedQueue(rows,rowMarkup) {
 ...
 return [...groups.values()].map(g=>`<details class="rg-unit"><summary><span><b>${esc(g.listing.property_name || g.listing.title || 'Property')}</b></span><span>${esc(g.listing.unit || '—')}</span><span>${g.rows.length} application group${g.rows.length===1?'':'s'}</span><span>${g.rows.filter(r=>r.next_step?.bucket==='attention').length} need attention</span></summary><div class="rg-unit-apps">${g.rows.map(rowMarkup).join('')}</div></details>`).join('');
}
export function rentalGroupMarkup(ctx,h) {
 ...
 const approved=row.status==='landlord_approved' || (row.status==='lease_sent' && !w.tenant_signature);
 ...
 const progression=approved ? `<p>${draft?.error ? esc(draft.error) : draft?.missing?.length ? 'The lease draft needs property information before signing.' : 'The lease draft is ready. Review it before sending for signatures.'}</p><div class="cw-actions"><a class="desk-button" href="#/leases/${esc(id)}">Review lease draft</a>${w.lease_preparation ? '<button class="primary" data-download-lease>Download lease draft</button>' : form('refresh_draft','','Refresh lease draft')}</div>${draft?.missing?.length ? `<details><summary>${draft.missing.length} missing lease fields</summary><ul>${draft.missing.map(f=>`<li>${esc(f)}</li>`).join('')}</ul></details>`:''}${signatures}`
  ...
  : closed ? h.primaryFor(ctx) : `<p>Each applicant completes their own documents and credit check. The system sends one email to the landlord when the whole group is ready.</p>${attention}<p class="cw-note">${members.some(m=>m.workspace?.screening_result?.status==='not_connected') ? 'Credit-check provider not connected. Record an external report for each applicant below.' : 'No manual recommendation is required.'}</p>${form('retry_delivery','','Check progress')}`;
 const person=(m,i)=>{
  const summary=group.summary.find(s=>s.id===m.id) || {},mc={...ctx,row:m,w:m.workspace || {},docs:h.documentSummary(m,ctx.types)};
  const checks=editable ? `<details class="cw-edit"><summary>Record external report / payment</summary>${h.checksForm(mc.w).replace('<form ',`<form data-member="${esc(m.id)}" `).replace('<button class="primary"',`<input type="hidden" name="member_id" value="${esc(m.id)}"><label>Score model<input name="score_model" value="${esc(summary.score_model==='Model not recorded'?'':summary.score_model)}" placeholder="As shown on the report"></label><button class="primary"`)}</details>`:'';
  const columns=applicantColumns(m,{...ctx,documentSummary:h.documentSummary},checks,summary,money);
  const selected=selectedApplicants.get(viewKey) || members[0]?.id;
  return `<section class="rg-person" data-person-panel="${esc(m.id)}" ${m.id!==selected?'hidden':''} aria-label="${esc(m.name)}"><div class="rg-pair">
 ...
 const stageLabel={sent_to_landlord:'Awaiting landlord decision',landlord_approved:draft?.missing?.length || draft?.error?'Lease needs information':'Lease ready for review',lease_sent:'Signatures in progress',lease_signed:'Lease completed',declined:'Not proceeding'}[row.status] || 'Collecting applications';
 ...
 const screens={
  applicants:`<div class="rg-applicant-toolbar"><nav class="rg-members" aria-label="Applicants">${members.map((m,i)=>`<button type="button" data-person="${esc(m.id)}" aria-pressed="${m.id===selectedApplicants.get(viewKey)}"><small>Applicant ${String.fromCharCode(65+i)}</small><b>${esc(m.name)}</b></button>`).join('')}${pending.map((i,n)=>`<div class="rg-invited"><small>Awaiting submission</small><b>${esc(i.name)}</b></div>`).join('')}</nav><div class="rg-reading-tools"><button type="button" data-expand-record>Expand all sections</button>${editable?'<button type="button" data-toggle-members>Manage applicants</button>':''}</div></div><div data-member-tools hidden>${groupTools}</div><div class="rg-household"><div class="rg-people">${members.map(person).join('')}</div></div>`,
...
export async function bindRentalGroup(host,ctx,reload,current) {
 ...
 if(host.querySelector('[data-merge-options]')) {
  try {
   ...
   const eligible=cases.filter(r=>r.id!==ctx.id && r.listing_id===ctx.row.listing_id && r.household?.members.length===1 && !r.household?.invitations.some(i=>!i.accepted) && !['sent_to_landlord','landlord_approved','lease_sent','lease_signed','declined'].includes(r.status));
   host.querySelector('[data-merge-options]').innerHTML=eligible.length ? eligible.map(r=>`<div class="rg-merge-choice"><b>${esc(r.name)}</b><p>${esc(r.email)} · ${esc(r.move_in)} · ${esc(r.responsible_email || 'Unassigned')}</p>${form('merge',`<input type="hidden" name="application_id" value="${esc(r.id)}"><input type="hidden" name="source_version" value="${r.workspace_version}"><label class="cw-check"><input type="checkbox" name="confirmed" required><span>These applicants intend to share one lease. Use this group’s agent, rent and dates.</span></label>`,'Join this application')}</div>`).join(''):'<p>No eligible independent applications for this unit.</p>';
