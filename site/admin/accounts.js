  ...
  try {
    ...
    async function detail(member) {
      ...
      panel.innerHTML = `<span class="k">${nameOf(member.role)} account</span><h2>${isNew ? `Add ${nameOf(member.role).toLowerCase()}` : esc(member.name || member.email)}</h2><p class="soft">${editable ? (member.role === "landlord" ? "Update contact details and account status. Registered properties stay bound." : "Update this person's details and access.") : "This account is read only for you."}</p>
        ...
        ${!isNew && editable && member.active ? '<button type="button" id="account-invite">Send invitation</button><button type="button" id="account-revoke-invite">Revoke pending invitation</button><p id="invite-status" role="status"></p>' : ""}
      ...
      if(revokeInvite)revokeInvite.onclick=async()=>{revokeInvite.disabled=true;try{await send(api,`/staff/${encodeURIComponent(member.email)}/revoke-invitation`,{});panel.querySelector('#invite-status').textContent='Pending invitation revoked. Existing accepted access is unchanged; suspend the account to remove access.';}catch(error){panel.querySelector('#invite-status').textContent=error.message;}finally{revokeInvite.disabled=false;}};
