...
export function openNewProperty({draftOwner, fields, api, escapeHtml: esc, onSaved, setStatus}) {
  ...
  const pairMarkup = pair => {
    ...
    return `<tr class="draft-pair"><th><label for="draft-pair-${pair.positive}">${esc(pair.label)}</label>${mode==='manual'?'':conflict?'<span class="import-flag">Conflicting imported choices · Select one</span>':sources.length?'<span class="import-detected">Detected</span>':''}</th><td><select id="draft-pair-${pair.positive}" data-import-pair="${pair.positive}"><option value="">Choose One</option>${(pair.order || ['yes','no']).map(option=>`<option value="${option}"${value===option?' selected':''}>${pair[option]}</option>`).join('')}</select><small class="draft-default-hint">Selecting one clears the other mark.</small>${[yes,no].map(row=>`<input hidden data-import-value="${row.id}" value="${esc(String(row.value))}">`).join('')}${sources.length?`<details class="import-source"><summary>View Source${conflict?' & Alternatives':''}</summary>${sources.map(s=>`<p>${esc(s.evidence)}</p>${s.location?`<small>${esc(s.location)}</small>`:''}`).join('')}</details>`:''}</td></tr>`;
  };
