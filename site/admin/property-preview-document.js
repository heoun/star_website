import {mountDocument,patchValues,showSections,clearHighlight} from './lease-doc.js';
...
const slots=()=>[...host.querySelectorAll('[data-lease-slot]')];
...
function paint(data){
 ...
 picker.replaceChildren();for(const id of ids)picker.append(new Option(field(id).label,id));picker.value=active;
...
try{
 ...
 await mountDocument(host,{onSlotClick:id=>{if(payload?.ids.includes(canonical(id))){active=canonical(id);index=0;picker.value=active;locate();}}});ready=true;
