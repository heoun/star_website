...
export async function secureAccount(host,post,enter,{manage=false,forceVerify=false}={}) {
  ...
  if(forceVerify||verifyExisting||enrollRequired) {
    ...
    if(!factor) {
      host.innerHTML='<h2>Protect your account</h2><p>Owner and Admin accounts require an authenticator. Add a time-based account in your authenticator app, then enter its six-digit code.</p><button class="primary" id="enroll">Set Up Authenticator</button><p class="status" role="status"></p>';
  ...
  if(state.setup_password||state.reset_password) {
    ...
    host.innerHTML=`<h2>${resetting?'Choose a new password':'Create your password'}</h2><p>${resetting?'Your identity has been verified. Save your new password, then sign in with it.':'Your email is verified. Choose a password for future sign-ins.'}</p><form><label>New password<input name="password" type="password" autocomplete="new-password" minlength="8" maxlength="200" required></label><label>Confirm password<input name="confirm" type="password" autocomplete="new-password" minlength="8" maxlength="200" required></label><button class="primary">Save Password</button><p class="status" role="status"></p></form>`;
    bind(async data=>{
  ...
  host.innerHTML=`<h2>Account security</h2><p>${esc(state.email)}</p><p>Two-step verification: ${state.verified?'Verified':'Not verified'}</p>${state.owner_account?`<h3>Your workspace roles</h3><p>Owner manages account access. Admin manages business operations.</p><button type="button" class="auth-secondary" data-role="owner">Open Owner Workspace</button>${state.admin_enabled?'<button type="button" class="auth-secondary" data-role="admin">Open Admin Workspace</button>':''}<form><label>Reason for ${state.admin_enabled?'revoking':'granting'} your Admin access<input name="reason" minlength="5" maxlength="1000" required></label><button class="primary">${state.admin_enabled?'Revoke':'Grant'} My Admin Role</button><p class="status" role="status"></p></form>`:''}<p><a href="/login/?reset=1">Reset Password</a></p><p><a href="/admin/">Back to Workspace</a></p><button type="button" class="auth-secondary" id="reauth">Verify Authenticator Again</button><p id="security-status" class="status" role="status"></p>`;
