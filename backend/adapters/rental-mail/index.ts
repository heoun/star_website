...
import { logoAttachment } from './logo.ts';
...
export function makeRentalMail(env:Record<string,any>,request:Request):RentalMail {
  async function send(to:string,subject:string,text:string,html:string,key:string) {
    const attachments=html.includes(`cid:${logoAttachment.content_id}`)?[logoAttachment]:[];
    const ok=await sendEmail(request,env,{from:'Star Realty <no-reply@starreusa.com>',to:[to],subject,text,html,...(attachments.length?{attachments}:{})},{idempotencyKey:key});
  ...
  return {
    async decision(root,members,key) {
      ...
      const button=(label:string,choice:string)=>`<a style="display:inline-block;padding:12px 18px;margin:8px 8px 8px 0;border-radius:8px;background:#111111;color:#ffffff;text-decoration:none" href="${esc(link(choice))}">${label}</a>`;
      const html=`<div style="font:16px Arial,sans-serif;color:#111111;background:#ffffff;max-width:680px;margin:auto;line-height:1.6"><p style="margin:0 0 24px"><img src="cid:star-real-estate-logo" alt="Star Real Estate" width="112" height="112" style="display:block;width:112px;height:112px;border:0;background:#ffffff"></p>${root.workspace?.test_run?`<p><b>Internal Test</b> · Run ${esc(root.id.slice(0,8))} · Synthetic screening</p>`:''}<h1 style="font-size:24px">Application ready for your decision</h1><h2>${esc(place)}</h2><p style="white-space:pre-line">${esc(facts)}</p><table style="width:100%;border-collapse:collapse"><thead><tr><th align="left">Applicant</th><th align="left">Credit score</th><th align="left">Annual income</th></tr></thead><tbody>${members.map(m=>`<tr><td style="padding:16px 8px;border-bottom:1px solid #dddddd">${esc(m.name)}<br><small>${esc(m.employment)}</small></td><td style="padding:16px 8px;border-bottom:1px solid #dddddd"><b>${esc(m.credit_score ?? 'No score returned')}${m.mock?' (mock)':''}</b><br><small>${esc(m.score_model)}<br>${esc(m.report_date.slice(0,10))}</small></td><td style="padding:16px 8px;border-bottom:1px solid #dddddd">${esc(m.annual_income || 'Not stated')}<br><small>${esc(m.income_source)}</small></td></tr>`).join('')}</tbody></table><p>${button('View details','details')}</p><p>${button('Agree to proceed','accept')}${button('Do not proceed','decline')}</p><p>Confirm with your landlord account. Your decision covers this whole application group and these terms. Agreeing starts lease preparation; it does not sign the lease.</p></div>`;
