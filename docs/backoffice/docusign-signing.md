...
选中文件的同时，该副本里由租约填入的每个值（租客与房东名称、地址、租金、日期、各 rider
字段等）以琥珀色标出，图例里是 Filled Values，条目下方写明标出了多少个值。标记只画在浏览器
渲染出的文字上（`site/admin/signing-value-highlight.js`），不改动保存的 DOCX、PDF、锚点或签字
坐标。副本本身没有哪些字是填进去的痕迹，来源是审核页自己渲染的带占位符模板：Worker 切出的
副本与模板段落一一对应、顺序一致（token 只加 run，不加段落），所以按 Worker 的切分位置对应
段落，再核对整段文字一致后才标；段落数或文字对不上就不标，并在条目下方提示有几个值未能标出。
逐租客副本按该租客自己的姓名、邮箱、通信地址核对。空值不标，同一值多处出现各标各处，一个值
跨多个 run 时每个 run 各一段标记。
