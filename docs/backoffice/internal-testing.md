...
5. On the **Credit Screening** page, confirm the identity fields, authorize the
   ...
   simulator.
6. A complete scored report plus complete materials sends the applicant's own
   ready-for-review confirmation and triggers the normal rental workflow and
   landlord summary email. No-score and failure scenarios block automatic
   sharing; the no-score applicant is still confirmed, the failed one is not. The applicant can refresh or leave the page; the scheduler
...
`npm run test:email` checks the actual Resend JSON payload, the shared property
label (`mailPlace`, which never repeats a building and unit the listing title
already spells out) and the ready-for-review confirmation template; `test:rentals`
asserts branding on the confirmation emitted through a real application-route
submission and reconciliation. Use
`EMAIL_RECEIPT_PREVIEW=/tmp/application-receipt.html node scripts/test-rental-intake.mjs`
to render that isolated fixture confirmation without sending a real email.

### Applicant confirmation

With the rental workflow on, submitting the form no longer mails the applicant.
The submission enrols the application (`workspace.ready_notice = queued`, stored
with the row, including a roommate who submits first), and reconciliation sends
one **Application received · {place}** confirmation per applicant once their own
fee, required documents and screening report are complete, whatever the rest of
the group is still waiting on. It promises a review, not an approval, and does
not describe the report; a documented no-score outcome is complete from the
applicant's side, while a failed or pending report holds it. Delivery is
recorded as `sent`, `preview` or `failed`; a failure is retried by later
reconciliations with the same idempotency key after a growing pause, and is
still owed after the case moves on to approval or signing: the scheduler finds
it through the queued/failed notice rows alone, so closed cases are not polled.
A declined case or member is never mailed; its unsent notice is recorded as
`cancelled` on the next reconciliation, which also takes the case out of that
query. Rows without the marker predate enrolment and are never mailed, so
enabling this code does not backfill confirmations.

With `RENTAL_AUTOMATION=off` there is no fee, document or screening readiness
step, so no applicant email is sent at all; the former submission-time receipt
with its upload checklist has been removed. The office submission notification
