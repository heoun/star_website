...
2026-09-09 已通过：113 项工作台检查、82 项账户/入驻 HTTP 检查、35 项账户/入驻 SQL 检查、89 项旧权限回归、103 项申请检查、35 项 Word/租约检查、工作台 SQL 回归、构建、类型检查、依赖边界检查及 v2 内存 smoke。旧账户测试中“普通 Admin 可修改其他 Admin”的断言已被新治理测试替代。浏览器此前验证了分配、Agent 范围、审核/推荐、房东确认后刷新保留、建议审批发布；本次验证了 Admin 同级账户只读、Owner 授权/撤权、邀请、草稿刷新、两栋物业提交/批准、自动绑定、新房东仅见自己的物业，以及桌面和 390px 手机布局。当时的可选 Playwright 回归脚本未在该次运行；2026-09-14 已由 `npm run test:rentals:ui` 的自动申请组场景取代，详见对应开发检查点。
