...
Property settings in both roles include a read-only template preview beside the current step (stacked on narrow screens). One occurrence is shown at a time, with previous/next and a location directory. The preview patches unsaved input locally without saving or calling signing services. Tenant/listing fields remain source-labelled placeholders. Locations use template section numbers rather than claiming final PDF page numbers. The existing rental document workspace keeps its own preview.

The property editor preserves the connected preview iframe across step changes, dialogs and saves. Only surrounding editor nodes are replaced; the iframe is never detached. Preview input updates patch changed slots only and rebuild navigation only when the focused field or step changes. Browser regression checks assert the same preview document and zero template reloads across an editing cycle.
