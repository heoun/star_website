...
Dev and production use separate Auth databases: a Dev account never grants production access. Dev workspace invitations identify the test environment in their subject and body; the Dev login page points out the separate accounts. Production users receive their own production invitation and activation. An Owner/Admin-authorized workspace invitation can be emailed to its single active directory member on Dev, even outside the synthetic rental-test inbox list. This scoped exception does not permit ordinary notifications or additional recipients outside that list.
