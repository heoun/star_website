import {alphabet} from './demo-names.mjs';
import {seedListingMedia} from './demo-listing-media.mjs';
...
export const MOCK_PREVIEW_TENANCY = {
  "tenant.names": "Applicant Preview", "tenant.email": "preview-tenant@example.test",
...
function mockApplication(listing, index, applicationId) {
  const student = index % 2 === 1;
  return {
    first_name: "Applicant", last_name: alphabet(index + 5), name: `Applicant ${alphabet(index + 5)}`, email: `applicant-${applicationId}@example.test`,
    ...
    current_employer: { employer: "Example Design Studio", position: "Product Designer", start: "06/01/2023", supervisor_name: "Supervisor A", supervisor_phone: "212-555-0162", supervisor_email: "supervisor@example.test" },
    ...
    employment_history: [{ employer: "Example Research Group", position: "Research Associate", start: "07/01/2020", end: "05/31/2023", income: "85000", supervisor_name: "Previous Supervisor A", supervisor_phone: "212-555-0163", supervisor_email: "previous-supervisor@example.test" }],
    rental_history: [{ landlord_name: "Example Former Landlord LLC", address: `${10 + index} Example Lane, Apt 3, New York, NY 10001`, contact: "Previous Landlord A", landlord_phone: "212-555-0164", landlord_email: "previous-landlord@example.test", start: "07/01/2023", end: "09/30/2026", monthly_rent: "2400" }],
    reference_contacts: [{ name: "Reference A", relationship: "Former colleague", phone: "212-555-0165", email: "reference-one@example.test" }, { name: "Reference B", relationship: "Professional reference", phone: "212-555-0166", email: "reference-two@example.test" }],
    emergency_contacts: [{ name: "Emergency Contact A", relationship: "Sibling", phone: "212-555-0167", email: "emergency-contact@example.test" }],
    roommates: [{ first_name: "Applicant", last_name: alphabet(index + 12), phone: "212-555-0168", email: `roommate-${applicationId}-1@example.test` }],
...
export async function completeDemoState(state) {
  repairDraftDemoApplications(state);
  ...
  for (const [i, b] of state.buildings.entries()) {
    ...
    if (!landlord && firstSeed) {
      landlord = { email: `property-${b.id}@example.test`, name: `Landlord ${alphabet(i)}`, role: "landlord", active: true, property_ids: [b.id], account_version: 0 };
      state.staff.push(landlord);
    }
    landlord ||= { email: b.landlord_signer_email || "landlord@example.test", name: "Landlord A" };
  ...
  for (const [i, l] of state.listings.entries()) {
    ...
    if (l.published && !state.applications.some(a => a.listing_id === l.id) && !seeded.listings[`${l.id}:application`]) {
      const id=crypto.randomUUID();
      state.applications.push({ id, ...mockApplication(l, i, id) });
    }
    if(l.published) seeded.listings[`${l.id}:application`] = true;
  }
  ...
  for (const [i, a] of state.applications.entries()) {
    ...
    fillOnce("applications", a.id, a, mockApplication(listing, i, a.id));
...
// Earlier seeds created a preview applicant even for a never-published draft.
// Remove only that identifiable local seed shape. Real submissions and progressed
// cases remain valid historical records when a listing is subsequently unlisted.
export function repairDraftDemoApplications(state) {
  if(state.demo_draft_application_seed===1)return;
  const removed=new Set(state.applications.filter(a=>{
    const listing=state.listings.find(l=>l.id===a.listing_id);
    return listing && !listing.published && a.created_at===at && !a.submitted &&
      ['new','review'].includes(a.status) && !a.workspace?.recommendation && !a.workspace?.landlord_decision &&
      !!state.demo_seed?.listings?.[`${listing.id}:application`];
  }).map(a=>a.id));
  if(removed.size){
    const docs=state.documents.filter(d=>removed.has(d.application_id));
    for(const d of docs)if(d.path?.includes('/mock/'))delete state.files[d.path];
    state.documents=state.documents.filter(d=>!removed.has(d.application_id));
    state.applications=state.applications.filter(a=>!removed.has(a.id));
    state.emails=(state.emails || []).filter(m=>![...removed].some(id=>JSON.stringify(m).includes(id)));
    for(const listing of state.listings)if(!listing.published && !state.applications.some(a=>a.listing_id===listing.id))delete state.demo_seed.listings[`${listing.id}:application`];
  }
  state.demo_draft_application_seed=1;
}
