...
try{
 ...
 const activation=await call('workspace','workspace-code',{email:same,invite},'member');eq(activation.status,200);eq(mails.length,0);
 eq((await activation.json()).activation_ready,true);const activationCode='';
 await db.query('update workspace_invitations set revoked_at=now() where email=$1',[same]);
 eq((await call('workspace','verify-reset',{code:activationCode,password:'blocked-password',activation:true,invite},'member')).status,403);
 await db.query("update workspace_invitations set revoked_at=null,expires_at=now()-interval '1 day' where email=$1",[same]);
 eq((await call('workspace','workspace-code',{email:same,invite},'member')).status,403);
 eq((await call('workspace','verify-reset',{password:'blocked-password',activation:true,invite},'member')).status,403);
 await db.query("update workspace_invitations set expires_at=now()+interval '1 day' where email=$1",[same]);
 eq((await call('applicant','verify-reset',{password:'blocked-password',activation:true,invite},'member')).status,403);
 eq((await call('workspace','verify-reset',{password:'blocked-password',activation:true,invite:'b'.repeat(64)},'member')).status,403);
 ...
 eq((await call('workspace','verify-reset',{code:activationCode,password:'new-password',activation:true,invite},'member')).status,403);
