...
if(process.argv.includes('--ui')){
 ...
 for(const width of [900,390]){await page.setViewportSize({width,height:1000});assert.equal(await page.locator('.rg-contact-card').count(),3);assert(await page.getByText('Example Bakery',{exact:true}).isVisible());assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),true);await page.screenshot({path:`/tmp/star-applicant-display-${width}.png`,fullPage:true});}
