...
const users = new Map(), tokens = new Map(), codes = new Map(), requests = [], revokedSessions = new Set(), refreshSessions = new Map(), tokenSessions = new Map();
...
const issue = (row, session = crypto.randomUUID()) => {
  const token = crypto.randomUUID(), refresh = crypto.randomUUID();
  tokens.set(token, row); tokenSessions.set(token, session); refreshSessions.set(refresh, {row, session});
  return { user: row, access_token: token, refresh_token: refresh };
};
...
globalThis.fetch = async (input, init = {}) => {
  ...
  if (url.pathname === "/auth/v1/token") {
    if (url.searchParams.get("grant_type") === "refresh_token") {
      const active = refreshSessions.get(body.refresh_token);
      if (!active || revokedSessions.has(active.session)) return reply({message:"Invalid refresh token"},401);
      refreshSessions.delete(body.refresh_token);
      return reply(issue(active.row, active.session));
    }
    const row = users.get(body.email);
    return row && row.password === body.password ? reply(issue(row)) : reply({message:"Invalid login credentials"},401);
  }
  if (url.pathname === "/auth/v1/user") {
    const row = tokens.get(token); if (!row || revokedSessions.has(tokenSessions.get(token))) return reply({},401);
  ...
  if (url.pathname === "/auth/v1/logout") { const session = tokenSessions.get(token); if (session) revokedSessions.add(session); tokens.delete(token); return reply({}); }
