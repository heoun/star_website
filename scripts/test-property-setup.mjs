...
import {reviewIssues} from '../site/admin/lease-workspace.js';
...
for(const empty of ['',null,' '])assert.equal(resolve({'rent.due_day':'1'},{},{'rent.due_day':empty})['rent.due_day'],'1');
assert.equal(resolve({'rent.due_day':'1'},{},{'rent.due_day':'5'})['rent.due_day'],'5');
const sharedReview={missing:new Set(),byId:new Map(),checked:new Set(),dirty:new Set(),values:{},landlordEmail:'owner@example.test',caseRow:{household:{members:[{name:'One',email:'shared@example.test'},{name:'Two',email:'shared@example.test'}]}},signing:{configuration:{enabled:true,reviewOnly:true}}};
assert.deepEqual(reviewIssues(sharedReview),[]);
sharedReview.signing.configuration.reviewOnly=false;
assert.ok(reviewIssues(sharedReview).some(i=>i.label==='Each signer needs a different email address'));
