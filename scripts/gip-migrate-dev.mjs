...
if(command==='plan'){
 ...
 const unboundStaff=staff.filter(s=>s.active&&s.access_state==='active'&&!s.user_id&&!s.auth_user_id&&!users.some(u=>u.email===s.email)).map(s=>s.email);
 save({project,source:env.SUPABASE_URL,createdAt:new Date().toISOString(),accounts,applications,drafts,unboundStaff});
...
const pending=(plan.unboundStaff||[]).map(email=>`with pending as (update public.staff set access_state='invited' where email=${q(email)} and active and access_state='active' and user_id is null and auth_user_id is null and not exists(select 1 from auth.users where lower(email)=${q(email)} and email_confirmed_at is not null) returning email) insert into public.identity_audit(acting_role,action,details) select 'operator','gip_migration_require_invitation',jsonb_build_object('email',email,'reason','No verified source identity') from pending;`);
const sql=['begin;','lock table public.applications,public.rental_drafts,public.staff in share row exclusive mode;',...pending,...Object.entries(tenants).map(([realm,tenant])=>`insert into public.gip_auth_realms(realm,project_id,tenant_id) values(${q(realm)},${q(project)},${q(tenant)}) on conflict(realm) do nothing;`),...plan.accounts.map(a=>`select public.migrate_gip_identity(${q(a.oldSubject)},${q(a.newSubject)},${q(a.email)},${q(a.realm)},${q(project)},${q(a.tenant)});`),`do $$ declare affected integer; begin update public.gip_auth_realms set enabled=true where project_id=${q(project)} and ((realm='workspace' and tenant_id=${q(tenants.workspace)}) or (realm='applicant' and tenant_id=${q(tenants.applicant)})); get diagnostics affected=row_count; if affected<>2 then raise exception 'Expected exactly two approved Dev realms'; end if; end $$;`,"notify pgrst,'reload schema';",'commit;'].join('\n')+'\n';
