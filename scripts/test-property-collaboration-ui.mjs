...
try{
 ...
 await g.locator('[data-settings-save="sprinkler"]').click();
 await g.locator('.property-steps [data-property-step="signing"]').click();
 await g.locator('.property-steps [data-property-step="smoking"]').click();
 const otherToggle=g.locator('[data-setting="smoking.other_areas"]');
 const otherDescription=g.locator('[data-setting="smoking.other_areas_text"]');
 await otherToggle.check();
 assert.equal(await otherDescription.getAttribute('aria-required'),'true');
 const writesBeforeSmoking=agentWrites.length;
 await g.locator('[data-settings-save="smoking"]').click();
 assert.equal(await otherDescription.getAttribute('aria-invalid'),'true');
 assert.equal(agentWrites.length,writesBeforeSmoking,'Selected other smoking areas need a description before saving');
 await otherDescription.fill('Shared roof terrace');
 assert.equal(await otherDescription.getAttribute('aria-invalid'),null);
 await g.locator('[data-settings-save="smoking"]').click();
 await g.getByText('Saved 2 values to the draft. Admin approval is required.',{exact:true}).waitFor();
 await otherToggle.uncheck();
 assert.equal(await otherDescription.getAttribute('aria-required'),'false');
 await otherDescription.fill('');
 await g.locator('[data-settings-save="smoking"]').click();
 await g.locator('.property-steps [data-property-step="signing"]').click();
 ...
 assert.equal(await justification.getAttribute('aria-required'),'true');
 const writesBeforeIncrease=agentWrites.length;
 await g.locator('[data-settings-save="good_cause"]').click();
 assert.equal(await justification.getAttribute('aria-invalid'),'true');
 assert.equal(agentWrites.length,writesBeforeIncrease,'Above-threshold increase requires B-1 justification');
 await justification.fill('Example justification');
 await g.locator('[data-settings-save="good_cause"]').click();
 await g.getByText('Saved 3 values to the draft. Admin approval is required.',{exact:true}).waitFor();
 await increase.selectOption('no');
 assert.equal(await justification.getAttribute('aria-required'),'false');
