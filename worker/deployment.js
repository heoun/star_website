...
export function deploymentResponse(response,env,request) {
  ...
  if(result.headers.get('Content-Type')?.includes('text/html') && typeof HTMLRewriter!=='undefined') {
    // The outer workspace already identifies Dev; embedded previews need no duplicate badge.
    result.headers.append('Vary','Sec-Fetch-Dest');
    if(['iframe','frame'].includes(request?.headers.get('Sec-Fetch-Dest')))return result;
