...
export async function splitSigningDocuments(docx,originalXml,values,signers,tenantValues={}){
 ...
 const nodes=elements(body),finalSection=nodes.pop();
 ...
 for(let i=0;i<starts.length;i++){
  ...
  for(const tenant of INDIVIDUAL_NOTICES.has(d.id)?signers.filter(s=>s.role==='tenant'):[null]){
   ...
   const own=last?xml.slice(0,last.index)+xml.slice(last.index+last[0].length):xml;
   result.push({documentId:String(result.length+1),layout:d.id,tenantRecipientId:tenant?.recipientId || null,name:d.name+(tenant?` — ${tenant.name}`:''),xml:own,
    bytes:await replaceEntry(entries,'word/document.xml',head+own+section+tail)});
  }
 }
